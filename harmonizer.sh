...
Options:
  ...
  -l, --chromlist     Comma-separated list of chromosomes to harmonize (default=auto-detect)
...
BUILD="${BUILD-}"
REF_DIR="${REF_DIR-}"
CHROMLIST_OVERRIDE="${CHROMLIST_OVERRIDE-}"
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --chromlist)    CHROMLIST_OVERRIDE="${2:-}"; shift 2 ;;
    --chromlist=*)  CHROMLIST_OVERRIDE="${1#*=}"; shift ;;
    -h|--help)      usage; exit 0 ;;
    --)             shift; break ;;
...
OPTIND=1
while getopts ":i:b:t:r:c:l:h" opt; do
  case "$opt" in
    i) SUMSTATS="$OPTARG" ;;
    b) BUILD="$OPTARG" ;;
    t) THRESHOLD="$OPTARG" ;;
...
to_gwas_ssf() {
  ...
  # Build minimal GWAS-SSF (Neale/PLINK2 autodetect)
  ...
  # Compute md5 for sidecar (needed by gwas_metadata.py model)
  MD5="$(md5sum < "$OUT_GZ" | awk '{print $1}')"

  # Sidecar YAML with md5sum included
  cat > "$OUT_YAML" <<YAML
# Study meta-data
...
chromlist_from_ssf() {
  ...
  # Extract unique chromosome values from the 'chromosome' column, normalize:
  # - strip leading 'chr'
  # - map 23->X, 24->Y, 26->MT (seen in some tools)
  ...
  # Fallback: if nothing detected, keep full default list
  if [[ ${#have[@]} -eq 0 ]]; then
...
if [[ -n "${CHROMLIST_OVERRIDE-}" ]]; then
  CHROMLIST="$CHROMLIST_OVERRIDE"
  echo "Chromosomes (override): $CHROMLIST"
else
